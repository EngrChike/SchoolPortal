    ...
    <div className="w-full max-w-4xl mx-auto px-2 sm:px-4">
      ...
      {!isResultUnlocked ? (
        ...
        <>
          ...
          <div className="bg-white border border-slate-200/85 rounded-3xl p-6 sm:p-8 shadow-sm relative print-sheet-node overflow-hidden space-y-5 font-sans">
            ...
            <div className="flex flex-col items-center justify-center text-center border-b border-slate-100 pb-4">
              <div className="h-14 w-14 mb-2 overflow-hidden flex items-center justify-center print-image-container">
            ...
            {/* Student Info Box */}
            <div className="border border-slate-200 rounded-2xl p-4 bg-white flex flex-col sm:flex-row justify-between gap-3 text-xs">
            ...
            <div className="border border-slate-200 rounded-2xl overflow-hidden bg-white">
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse min-w-[600px]">
                  <thead>
                    <tr className="border-b border-slate-200 text-[10px] font-black uppercase text-slate-800 bg-slate-50/50">
                      <th className="py-3 px-4 w-1/6">Module Code</th>
                      <th className="py-3 px-4 w-2/6">Module Description</th>
                      <th className="py-3 px-4 text-center">CA (30)</th>
                      <th className="py-3 px-4 text-center">Mid Sem (20)</th>
                    ...
                    {performanceRecords.map((rec, i) => {
                      ...
                      return (
                        <tr key={i} className="hover:bg-slate-50/30 transition-colors">
                          <td className="py-3 px-4 font-bold text-indigo-600 uppercase">{displayCourseCode}</td>
                          <td className="py-3 px-4 font-black text-slate-900">{courseName}</td>
                          <td className="py-3 px-4 text-center text-slate-600">{displayCA}</td>
                          <td className="py-3 px-4 text-center text-slate-600">{displayMid}</td>
                          <td className="py-3 px-4 text-center text-slate-600">{displayExam}</td>
                          <td className="py-3 px-4 text-center font-black text-slate-900">{totalScore}</td>
                        </tr>
                      );
                    })}
                  </tbody>
            ...
            <div className="border border-slate-200 bg-slate-50/60 rounded-2xl p-3.5 text-slate-800 text-center">
            ...
            <div className="pt-4 border-t-2 border-slate-200 grid grid-cols-2 gap-6 items-end">
              <div className="flex flex-col items-start">
                <div className="h-14 w-28 flex items-center justify-center mb-1.5 relative print-image-container">
                ...
                <div className="w-full max-w-[140px] print-stamp-box border border-slate-300 rounded-xl py-1.5 px-2 bg-slate-50 text-center shadow-xs">
                  <span className="text-[8px] uppercase font-bold text-slate-400">Institutional Stamp</span>
                </div>
              </div>
              
              <div className="flex flex-col items-end text-right">
                <div className="h-14 w-32 flex items-center justify-end mb-1.5 print-image-container">
                ...
                <div className="w-full max-w-[160px] print-signature-line border-b-2 border-slate-400 pb-1.5 text-center">
                  <p className="text-[11px] font-black text-slate-800">Registrar Administration</p>
                </div>
                <span className="text-[8px] uppercase font-bold text-slate-400 mt-1">Authorized Signature Validation</span>
              </div>
            </div>
