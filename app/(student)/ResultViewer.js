...
export default function ResultViewer({
  ...
  return (
    <div className="w-full">
      ...
      {/* Print rules that force zero margins and scale the document to fit 1 page */}
      ...
      {!isResultUnlocked ? (
        ...
        <>
          ...
          <div className="bg-white border border-slate-200/85 rounded-3xl p-5 shadow-sm relative print-sheet-node overflow-hidden">
            ...
            {/* Header Section */}
            <div className="border-b-2 border-slate-800 pb-2.5 mb-2.5 flex justify-between items-center">
              <div className="flex items-center space-x-3">
                <div className="h-10 w-10 overflow-hidden flex items-center justify-center relative flex-shrink-0">
              ...
              <div className="text-right border-r-2 border-indigo-600 pr-2.5">
                <span className="text-[7px] font-black text-indigo-600 uppercase tracking-widest block">Transcript Identity</span>
                <span className="text-[10px] font-mono font-bold text-slate-700 block">{regNumber || "UNALLOCATED"}</span>
              </div>
            </div>

            {/* Meta Info Grid */}
            <div className="grid grid-cols-4 gap-2 p-2 rounded-lg bg-slate-50 border border-slate-200/60 mb-2.5 text-[10px]">
              <div>
                <span className="block text-[7px] uppercase font-black text-slate-400">Student Name</span>
                <p className="font-bold text-slate-800 truncate">{fullName || "N/A"}</p>
              </div>
              <div>
                <span className="block text-[7px] uppercase font-black text-slate-400">Section Node</span>
                <p className="font-bold text-slate-800 truncate">{studentSection || "N/A"}</p>
              </div>
              <div>
                <span className="block text-[7px] uppercase font-black text-slate-400">Class Level</span>
                <p className="font-bold text-slate-800 truncate">{classLevel || "N/A"}</p>
              </div>
              <div>
                <span className="block text-[7px] uppercase font-black text-slate-400">Cummulative Avg</span>
                <p className="font-mono font-black text-indigo-600 bg-indigo-50 px-1 py-0.5 w-fit rounded">{overallAverageScore}%</p>
              </div>
            </div>
            ...
            <div className="mb-2.5">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b-2 border-slate-700 text-[8px] font-black uppercase text-slate-400">
                    <th className="py-1 px-1.5">Code</th>
                    <th className="py-1 px-1.5">Course Module Title</th>
                    <th className="py-1 px-1.5 text-center">CA (40)</th>
                    <th className="py-1 px-1.5 text-center">Exam (60)</th>
                    <th className="py-1 px-1.5 text-center">Total (100)</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 text-[10px] text-slate-700">
                  {performanceRecords.map((rec, i) => {
                    ...
                    return (
                      <tr key={i}>
                        <td className="py-1 px-1.5 font-mono font-bold text-indigo-600 uppercase">{rec.courses?.code || "N/A"}</td>
                        <td className="py-1 px-1.5 font-black text-slate-800 truncate max-w-[180px]">{rec.courses?.name || "N/A"}</td>
                        <td className="py-1 px-1.5 text-center font-mono text-slate-600">{compositeCA}</td>
                        <td className="py-1 px-1.5 text-center font-mono text-slate-600">{exam}</td>
                        <td className="py-1 px-1.5 text-center font-bold font-mono text-slate-900 bg-slate-50">{totalGrade}%</td>
                      </tr>
                    );
                  })}
                </tbody>
            ...
            <div className="mb-2.5 border border-slate-200 bg-white rounded-lg p-2 text-slate-800 text-center">
              <div className="text-[9px] font-black text-indigo-900 uppercase mb-0.5">Official Attestation Profile</div>
              <p className="text-[9px] text-slate-600 leading-tight max-w-xl mx-auto">
            ...
            <div className="pt-2 border-t border-slate-200 grid grid-cols-2 gap-4 items-end">
              <div className="flex flex-col items-start">
                <div className="h-9 w-20 flex items-center justify-center mb-0.5 relative">
                  {schoolStamp && <img src={schoolStamp} alt="Official Stamp" className="max-h-full object-contain" />}
                </div>
                <div className="w-full max-w-[120px] print-stamp-box border border-slate-300 rounded py-0.5 px-2 bg-slate-50 text-center">
                  <span className="text-[6px] uppercase font-bold text-slate-400">Institutional Stamp</span>
                </div>
              </div>
              <div className="flex flex-col items-center text-center">
                <div className="h-9 w-24 flex items-center justify-center mb-0.5">
                  {adminSignature && <img src={adminSignature} alt="Signature" className="max-h-full object-contain" />}
                </div>
                <div className="w-full max-w-[140px] print-signature-line border-b border-slate-400 pb-0.5">
                  <p className="text-[9px] font-black text-slate-800">Registrar Administration</p>
                </div>
                <span className="text-[6px] uppercase font-bold text-slate-400 mt-0.5">Authorized Signature Validation</span>
              </div>
            </div>
